...
class Solution {
	unordered_map <Node*, Node*> visitedList;
...
public:
	Node* cloneGraph(Node* node) {
		...
		if (visitedList.count(node)) {
			return visitedList[node];
		}

		Node* current = new Node(node->val);

		visitedList[node] = current;

		for (int i = 0; i < node->neighbors.size(); i++) {
			current->neighbors.push_back(cloneGraph(node->neighbors[i]));
		}
