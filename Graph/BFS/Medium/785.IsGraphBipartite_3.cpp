	...
	bool BFS(int startIndex) {
		...
		while(!myQueue.empty()) {
			...
			for (int i = 0; i < adjacency[hand].size(); i++) {
				// not colored yet
				if (-1 == visited[adjacency[hand][i]]) {
					myQueue.push(adjacency[hand][i]);
					//coloring the opposite
					visited[adjacency[hand][i]] = !(visited[hand]);
				} else {
					// colored
					// check the color
					if (visited[hand] == visited[adjacency[hand][i]]) {
						// if colors are same
						// return false
						return false;
					}
