...
class Solution {
	var robbed = Array(repeating: -1, count: 101)

	func maxValue(x: Int, y: Int) -> Int {
		return x > y ? x : y
	}

	func dpRob(_nums: [Int], index: Int) -> Int {
		if index == 0 {
			return nums[index]
		}
		...
		robbed[index] = maxValue(dpRob(nums, index: index - 1), dpRob(nums, index: index - 2) + nums[index])

		return robbed[index]
	}
 
	func rob(_ nums: [Int]) -> Int {
		return dpRob(nums, index: nums.count - 1)
	}
}